...
where $N_{\text{air}} = 512 \times \text{expansion}$ is the on-air bit count and $P_{TX,DC}$ is the DC power drawn by the transmitter chain (default 10 W). Reception costs $E_{RX} = P_{RX,DC} \cdot t_{\text{frame}}$ (default 2.5 W). All values in this section are generated by `scripts/energy_audit.py`.

For Conv+RS on the 12.5 kHz control channel at 12.5 kbps:

$$t_{\text{ctrl}} = \frac{512 \times 2.287}{12\,500} \approx 93.68\,\text{ms/frame}$$

For the 25 kHz bulk channel:

$$t_{\text{bulk-frame}} = \frac{512 \times 2.287}{25\,000} \approx 46.84\,\text{ms/frame}$$

### 6.2 Correction Snapshot

One correction cycle (requester + $N$ neighbours):

$$t_{\text{snap}} \approx (1 + N) \cdot t_{\text{ctrl}} + 2\,t_{\text{prop}}, \quad t_{\text{prop}} = d/c$$

For $N = 8$ neighbours at 1000 km ($t_{\text{prop}} \approx 3.3$ ms):

$$t_{\text{snap}} \approx 9 \times 93.68 + 2 \times 3.3 \approx 850\,\text{ms} \ll 5\,\text{s timer}$$

The requester spends one TX frame and $N$ RX frames:

$$E_{\text{requester}} = t_{\text{ctrl}}\left(P_{TX} + N P_{RX}\right) \approx 2.81\,\text{J}$$

Network energy includes requester TX/RX plus neighbour RX/TX:

$$E_{\text{network}} = t_{\text{ctrl}}\left((1+N)P_{TX} + 2N P_{RX}\right) \approx 12.18\,\text{J per correction event}$$

At 24 requester-initiated corrections/day, the requester's battery spends $67.4$ J $=0.0187$ Wh. The network total for those events is $292$ J $=0.0812$ Wh.

### 6.3 Bulk Relay / Borrow Dump

For a 1 MiB payload, compression ratio 3x, 45 bytes payload per frame, 25 kHz channel, GMSK Conv+RS at 25 kbps:
...
$$t_{\text{bulk}} = 7\,776 \times 46.84\,\text{ms} \approx 364\,\text{s} \approx 6.07\,\text{min}$$

This fits within a typical 15-minute LEO LoS window. Total energy:

$$E_{\text{bulk}} = t_{\text{bulk}} \times (P_{TX} + P_{RX}) = 364 \times 12.5 \approx 4\,552\,\text{J} \approx 1.26\,\text{Wh}$$

On a satellite generating 300 Wh/day with 200 Wh non-comms load, one 1 MiB relay consumes **0.42%** of daily generation, or **1.26%** of the remaining 100 Wh/day communications margin.
...
Correction responses dominate because each of $N$ neighbours must transmit. This motivates bounded neighbourhood sizes ($N \leq 8$ in the current spec). For periodic maintenance traffic, the paper assumes 12 heartbeat broadcasts/hour (one every 5 minutes). In an 8-neighbour mesh, one satellite spends about $0.225$ Wh/day transmitting its own heartbeats and receiving neighbour heartbeats, still below 0.1% of a 300 Wh/day generation budget.
...
$$t_{\text{ground}} \approx 364\,\text{s} \text{ (same 25 kbps PHY)}, \quad E_{TX,\text{s/c}} \approx 3\,642\,\text{J} \approx 1.01\,\text{Wh}$$
...
6. **A complete energetic model** showing 1 MiB relay at 1.26 Wh link total and 24 requester-initiated corrections/day at 18.7 mWh requester's battery cost — both well within CubeSat power constraints.
